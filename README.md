...
A 股全栈数据工具包 — 10+1 层架构 · 41 个端点 · 14 个数据源 · 零第三方数据封装依赖

一个自包含的 Skill 文件，把分散在 14 个数据源里的 A 股原始数据整合成 AI 编程助手直接能用的工具集。你不用再背 mootdx 的 K 线参数、东财的 PDF Referer 头、iwencai 的 X-Claw 鉴权——全部封装好了。
...
```
A 股全栈数据 · 十层+1架构 · V3.4.0
│  （优先级：mootdx/腾讯 不封IP 优先用；东财仅用于独有数据，已内置限流防封）
├── 行情层    mootdx + 腾讯财经 + 百度K线 + Ashare   K线(带MA5/10/20) + 五档盘口 + PE/PB/市值 + 指数/ETF + 指数分钟K线 ★v3.4
...
| **Ashare 指数分钟K线** | 上证/深证/创业板/科创50/沪深300/上证50/中证500 分钟级K线 · 新浪+腾讯双源（V3.4 新增） |
